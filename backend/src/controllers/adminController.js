...
async function createPendingUser(req, res, next) {
  try {
    ...
    const adminEmail = 'admin@unsaac.edu.pe'; // ejemplo, luego ajustar
    await sendAdminApprovalEmail(adminEmail, id);

    res.json({ message: 'Solicitud enviada, pendiente aprobación' });
  } catch (err) { next(err); }
}
...
async function approvePendingUser(req, res, next) {
  try {
    ...
    await pool.query(
      ...
      [userId, pendingUser.first_name, pendingUser.last_name, pendingUser.email, pendingUser.password_hash, pendingUser.roles, false]
    );

    // borrar de pending_users
    await pool.query(`DELETE FROM pending_users WHERE id=$1`, [pendingUserId]);

    // enviar correo al usuario para activación
    await sendUserActivationEmail(pendingUser.email, userId);
